...
final class WordlessChunkLiveTests: XCTestCase {
    ...
    func testAWordlessChunkOutranksAnOnTopicSentence() async throws {
        ...
        for (index, query) in queries.enumerated() {
            ...
            if query == queries[0] {
                // Запрос ровно про то, о чём предложение. На модели, где это
                // мерили, скобка обгоняла даже его — и это была цена одного
                // такого чанка в коллекции.
                //
                // **Насколько именно обгоняет, зависит от модели**.
                // Замер: на `qwen3-embedding-0.6b` скобка впереди, на
                // `qwen3-embedding-4b` уже нет — 0.4013 против 0.3683.
                // Требовать этого от любой модели значит проверять модель,
                // а не нашу нарезку; поэтому строгое сравнение — только там,
                // где его и мерили.
                if model.contains("0.6b") {
                    ...
                    print("""
                    «\(query)» на модели \(model): до скобки \(toJunk), \
                    до предложения по теме \(toReal) — \
                    \(toJunk < toReal ? "скобка впереди" : "скобка позади, модель крупнее и различает лучше")
                    """)
                }
            }
        }
