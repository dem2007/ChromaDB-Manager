...
public enum TextRelevance {
    ...
    /// Те же термы одним регулярным выражением — вместо перебора написаний
    /// (П6.1 ТЗ).
    ///
    /// **Замер на живом сервере** (chroma 1.4.4, коллекция из 5765 чанков,
    /// запрос из четырёх слов):
    ///
    /// | как спрашиваем | нашлось | время |
    /// |---|---|---|
    /// | `$or` из 12 написаний — как было | 3131 | 5079 мс |
    /// | одно `$regex` с `(?i)` | 3131 | 600 мс |
    /// | оно же с границей слова слева | 3110 | 820 мс |
    ///
    /// Полнота та же, время в восемь раз меньше. Отсюда и десятисекундный
    /// поиск, из-за которого текстовая стадия выглядела дорогой: пять секунд
    /// уходило на двенадцать подстрочных запросов вместо одного.
    ///
    /// **Граница слова — только слева, и это поправка к П6.1.** ТЗ предлагает
    /// `\bриск\b`, и на живых данных это отсекает 73 чанка из 104 — но
    /// отсекает «рисков» (57), «риски» (54), «риска» (17), то есть склонения
    /// того самого слова, а не мусор. Русский язык кончается справа, а не
    /// слева. Граница слева, наоборот, режет ровно мусор: у терма «рис» она
    /// убирает 577 чанков из 758 — «характеристики», «присваивается»,
    /// «Борисович», «жидкокристаллический» — и сохраняет «риск», «риски»,
    /// «рисков».
    ///
    /// **«ё» и «е» — классом символов**: подстрочный поиск так не умел вовсе,
    /// и это единственная часть П6.1, которая достаётся даром.
    ///
    /// Lookahead и lookbehind движок не поддерживает — проверено отказом на
    /// живом сервере, — но они здесь и не нужны.
    /// Имя оператора на проводе. Рядом с построением выражения, а не строкой
    /// посреди конвейера: `DocumentTextOperator` его не знает намеренно —
    /// выводить регулярные выражения в конструктор фильтров никто не просил.
    public static let regexOperator = "$regex"

    public static func regexPattern(of terms: [String]) -> String {
        // Граница приписывается **каждому** терму отдельно и только тогда,
        // когда он начинается с буквы или цифры. При выключенном разбиении на
        // слова термом становится весь запрос целиком — со скобками и
        // кавычками, — и общая граница перед «(в редакции)» стояла бы перед
        // скобкой, где её быть не может: выражение перестало бы совпадать
        // вовсе.
        let alternatives = terms
            .filter { !$0.isEmpty }
            .map { term -> String in
                let escaped = escapedForRegex(term)
                let starts = term.first.map { $0.isLetter || $0.isNumber } ?? false
                return starts ? "\\b" + escaped : escaped
            }
    ...
    /// Терм, приведённый к безопасному виду регулярного выражения.
    ///
    /// Экранируется **всё**, что движок мог бы принять за разметку: «C++»,
    /// «(в редакции)», «1.4.4» приходят от человека и означают ровно себя.
    /// После экранирования «е» и «ё» заменяются классом — по уже экранированной
    /// строке, чтобы не тронуть скобки самого класса.
    static func escapedForRegex(_ term: String) -> String {
    ...
    /// в интерфейсе нельзя.
    /// Способ, оставшийся от времён без `$regex`: см. `regexPattern(of:)`.
